...
import { getAcceleratorName } from '@nixrun/prefs/general/helper';
...
export class ShortcutRow extends Adw.ActionRow {
  ...
  constructor(ext: ExtensionBase) {
    ...
    const shortcutLabel = new Gtk4.ShortcutLabel({
      ...
      accelerator: this.settings.get_strv('global-shortcut-t')[0]!,
      valign: Gtk4.Align.CENTER,
      halign: Gtk4.Align.CENTER,
    });

    this.settings.connect('changed::global-shortcut-t', () => {
      shortcutLabel.set_accelerator(this.settings.get_strv('global-shortcut-t')[0]!);
    });
    ...
    this.connect('activated', () => {
      ...
      ctl.connect('key-pressed', (_, keyval, keycode, state) => {
        ...
        const globalShortcut = getAcceleratorName(keyval, keycode, mask, 'global-shortcut-t');

        if (globalShortcut === null) {
          return Gdk4.EVENT_STOP;
        }
        this.settings.set_strv('global-shortcut-t', [globalShortcut]);
        editor.destroy();
